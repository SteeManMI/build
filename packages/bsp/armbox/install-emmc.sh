...
dd if="${DEV_EMMC}" of=/root/u-boot-default.img bs=1M count=4

echo "Start create MBR and partittion"

parted -s "${DEV_EMMC}" mklabel msdos
parted -s "${DEV_EMMC}" mkpart primary fat32 1024M 1536M
parted -s "${DEV_EMMC}" mkpart primary ext4 1537M 100%

echo "Start restore u-boot"

dd if=/root/u-boot-default.img of="${DEV_EMMC}" conv=fsync bs=1 count=442
dd if=/root/u-boot-default.img of="${DEV_EMMC}" conv=fsync bs=512 skip=1 seek=1
...
sed -E "s/root=UUID=[^ ]*/root=LABEL=ROOT_EMMC/g" \
